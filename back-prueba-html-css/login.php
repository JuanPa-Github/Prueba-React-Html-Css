...
$method = $_SERVER['REQUEST_METHOD'];
    ...
    $mysqli = conectarDB();
     if(is_null($mysqli)){
      echo json_encode(array('conectado'=>false, 'error' => 'No se pudo conectar a la DB.'));
     };
        ...
        WHERE usuario = ?")) {
              ...
              if ($resultado->num_rows == 1) {
                  ...
                  if (password_verify($pas, $encriptado_db))
                  {
                      ...
                      echo json_encode(array('conectado'=>true,'usuario'=>$datos['usuario'], 
                      'nombre'=>$datos['nombre'],  'apellidos'=>$datos['apellidos'], 'id'=>$datos['id'], 
                      'idTipoUsuario'=>$datos['idTipoUsuario'], 'etiquetaTipoUsuario'=>$datos['etiquetaTipoUsuario'], 
                      'aprovado' => 'Felicitaciones!! Has Ingresado Correctamente'));
                    }
